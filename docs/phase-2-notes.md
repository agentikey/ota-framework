...
- **Generated `verbs.py` need to flow through L0b when a routine run is active.** The new `@verb` decorator (`ota_core/policy/__init__.py`) wraps calls and emits `tool_call.invoked` / `succeeded` / `failed` automatically *when an L0b run context is set*. The codegen output already imports `@verb`, so this works without regenerating. The bit that's still missing: the *integration_id* the call is targeting. The decorator can't know it yet — that's resolved at dispatch time by the binding resolver (Phase 3.3) plus the dispatch layer (Phase 3.5). When 3.5 lands, dispatch should call `L0bEnforcer.enforce_integration(integration_id)` and `enforce_scopes(integration_id, required_scopes)` *before* invoking the adapter. Hooks are in place; just wire them.

- **`dispatch()` placeholders still raise `NotImplementedError`** in [ota_connect/messaging/dispatch.py](../ota_connect/messaging/dispatch.py) and [ota_connect/email/dispatch.py](../ota_connect/email/dispatch.py). Phase 3.5 wires these to binding resolution + adapter invocation + error normalization. The L0b hooks above plug in here.

- **Cross-contract validation (Contract C ↔ D)** at routine-load is still deferred. The routine engine (Phase 2A.4) loads a `RoutineBundle` and validates the manifest *within* Contract C, but doesn't yet check that every `dependencies.integrations[].id` exists in the active `IntegrationRegistryManifest` and supports the declared scopes / binding_level. Phase 3's binding validator (work package 3.4) is the right home — it has both manifests in hand at install time.
